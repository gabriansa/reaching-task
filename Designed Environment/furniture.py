...
class Furniture(Agent):
    ...
    def init(self, furniture_type, directory, id, np_random, wheelchair_mounted=False):
        ...
        elif furniture_type == 'customtable':
            furniture = p.loadURDF(os.path.join(directory, 'CustomTable', 'custom_table.urdf'), basePosition=[0, 0, 0], baseOrientation=[0, 0, 0, 1], physicsClientId=id)
        elif furniture_type == 'lab_shelf':
            furniture = p.loadURDF(os.path.join(directory, 'lab_shelf', 'lab_shelf.urdf'), basePosition=[0, 0, 0], baseOrientation=[0, 0, 0, 1], physicsClientId=id)
        elif furniture_type == 'lab_table':
            furniture = p.loadURDF(os.path.join(directory, 'lab_table', 'lab_table.urdf'), basePosition=[0, 0, 0], baseOrientation=[0, 0, 0, 1], physicsClientId=id)
        elif furniture_type == 'ZED_camera':
            furniture = p.loadURDF(os.path.join(directory, 'ZED_camera', 'ZED_camera.urdf'), basePosition=[0, 0, 0], baseOrientation=[0, 0, 0, 1], physicsClientId=id)
        else:
            furniture = None

...
class Tool(Agent):
    ...
    def init(self, robot, task, directory, id, np_random, right=True, mesh_scale=[1]*3, maximal=False, alpha=1.0, mass=1):
        ...
        elif task in ['drinking', 'feeding', 'arm_manipulation', 'reaching', 'passing']:
            if task == 'drinking':
                ...
                visual_filename = os.path.join(directory, 'lab_marker', 'lab_marker.obj')
                collision_filename = os.path.join(directory, 'lab_marker', 'lab_marker_vhacd.obj')
                #mesh_scale = [0.001]*3
            elif task == 'passing':
                visual_filename = os.path.join(directory, 'lab_marker', 'lab_marker.obj')
                collision_filename = os.path.join(directory, 'lab_marker', 'lab_marker_vhacd.obj')
